...
def original_dataset(loaders, col):
    ...
    for idx1, loader in enumerate(loaders):
        for idx2, (r, d, a, y, r2, d2, a2, y2) in enumerate(loader):
            idx = idx1 + idx2
            i = torch.cat((r, d), 1)
...
def baseline_classifier(train_loader, valid_loader, test_loader, args, logger):
    ...
    if args.dataset == "law":
        col = ["input0", "input1", "input2", "input3", "input4", "input5",
               "input6", "input7", "input8", "input9", "input10", "y"]
    else:
        factual_df, counter_df, col = generated_dataset(["train"], args)
...
def s_classifier(args, logger):
    train_dat = np.load(os.path.join(args.save_path, "train.npz"))
    valid_dat = np.load(os.path.join(args.save_path, "valid.npz"))
    test_dat = np.load(os.path.join(args.save_path, "test.npz"))

    factual_x = np.concatenate([train_dat["input_real"], valid_dat["input_real"]], axis=0)
    counter_x = np.concatenate([train_dat["input_cf"], valid_dat["input_cf"]], axis=0)
    train_u = np.concatenate([train_dat["u"], valid_dat["u"]], axis=0)
    train_Y = np.concatenate([train_dat["y_real"], valid_dat["y_real"]], axis=0).squeeze()

    test_factual = test_dat["input_real"]
    test_counter = test_dat["input_cf"]
    test_u = test_dat["u"]
    test_Y = test_dat["y_real"]
    
    train_1 = (factual_x + counter_x) / 2
    train_2 = np.einsum('ij,ij->i', factual_x, counter_x)[:, np.newaxis]
    train_3 = np.exp(np.linalg.norm(factual_x - counter_x, axis=1))[:, np.newaxis]
    train_input = np.concatenate([train_1, train_2, train_3, train_u], axis=1)
    test_1 = (test_factual + test_counter) / 2
    test_2 = np.einsum('ij,ij->i', test_factual, test_counter)[:, np.newaxis]
    test_3 = np.exp(np.linalg.norm(test_factual - test_counter, axis=1))[:, np.newaxis]
    test_input = np.concatenate([test_1, test_2, test_3, test_u], axis=1)
    #train_input = np.concatenate([factual_x * counter_x, train_u], axis=1)
    #test_input = np.concatenate([test_factual * test_counter, test_u], axis=1)

    clfs = []
    if args.dataset == "law":
        clf = SM_LinearRegression()
        clf.fit(train_input, train_Y)
        test_pred = clf.predict(test_input)
        ...
        for name, clf in zip(["LR"], [LogisticRegression(penalty="l2", solver="liblinear")]):
            clf.fit(train_input, train_Y)
            test_pred = clf.predict(test_input)[:, np.newaxis]

            acc = accuracy_score(test_Y.ravel(), test_pred.ravel())
            logger.info("accuracy of {:.3s} whole -->    real: {:.4f}".format(name, acc))
            clfs.append(clf)
    
    for clf in clfs:
        test_counter_input = test_counter * test_dat["input"]
        #test_counter_input = np.concatenate([test_counter * test_dat["input"], test_u], axis=1)
        y_factual = clf.predict(test_input)
        y_counter = clf.predict(test_counter_input)
        a = test_dat["a"]
        if args.dataset == "law":
            r = test_dat["input_real"][:, :8]
        else:
        ...
        y_factual = clf.predict(curve_factual)
        y_counter = clf.predict(curve_counter)
        np.save(os.path.join(args.save_path, "{}_sclassifier_factual".format(args.use_real)), y_factual)
        np.save(os.path.join(args.save_path, "{}_sclassifier_counter".format(args.use_real)), y_counter)
