...
def test(test_loader, args, logger):
    ...
    with torch.no_grad():
        for idx, (r, d, a, y, r2, d2, a2, y2) in enumerate(test_loader):
            ...
            if args.use_label:
                if args.dataset == "law":
                    distance = torch.sum(torch.abs(y_p_val - y_p_counter_val))
                    _all += float(y_p_val.size(0))
                else:
        ...
        if args.use_label:
            if args.dataset == "law":
                acc = distance.cpu().detach().numpy() / _all
            else:
...
def generate_data(loader, args, dataset='train'):
    ...
    with torch.no_grad():
        for idx, (r, d, a, y, r2, d2, a2, y2) in enumerate(loader):
            ...
            if args.use_label:
                ...
                r_hard, d_hard, d_cf_hard = model.reconstruct_hard(u_prev, a.to(device))
    ...
    if args.use_label:
        np.savez(f_out_np, input=i_f, input_cf=i_cf, input_real=i_real, y=y_f, y_cf=y_cf, y_real=y_real, a=a_all, a_cf=a_cf_all, u=u)
    else:
        np.savez(f_out_np, input=i_f, input_cf=i_cf, input_real=i_real, y_real=y_real, a=a_all, a_cf=a_cf_all, u=u)
...
def generate_curve_data(loader, args, dataset="test"):
    ...
    if args.dataset == "law":
        ...
        u = torch.zeros((10000, args.ur_dim + args.ud_dim))
    else:
...
def law_test(test_loader, args, logger):
    device = args.device
    model_path = os.path.join(args.save_path, 'model.pth')
    test_model = torch.load(model_path)
    test_model.to(device)
    ...
    correct, _all, o1s, o2s, o3s, o4s, o1s_bin, o2s_bin, o3s_bin, o4s_bin, ys, ys_bin = \
        0, 0, None, None, None, None, None, None, None, None, None, None
    with torch.no_grad():
        for idx, (a, d, y) in enumerate(test_loader):
            loss_val, x_recon_loss_val, y_recon_loss_val, y_p_val, y_p_counter_val, u_kl_loss_val, vae_tc_loss_val, \
            fair_loss_val = test_model.calculate_loss(d.to(device), a.to(device), \
            y.to(device))

            # For saving the result:
            if args.tSNE == True:
                u_mu, u_logvar = test_model.q_u(d.to(device), a.to(device), y.to(device))
                u_prev = test_model.reparameterize(u_mu, u_logvar)
                ud_prev = u_prev
                ud = torch.cat((ud, ud_prev), 0) if idx != 0 else ud_prev
                a_all = torch.cat((a_all, a), 0) if idx != 0 else a
            ...
            mask_a = np.where(a == 1, -1, 1)
            #cf_effect = (y_cf_np - y_p_np) * mask_a
            cf_effect = np.abs(y_cf_np - y_p_np)
            #cf_bin = (np.greater(y_cf_np, 0.5).astype(int) - np.greater(y_p_np, 0.5).astype(int)) * mask_a
            cf_bin = np.abs(np.greater(y_cf_np, 0.5).astype(int) - np.greater(y_p_np, 0.5).astype(int))
...
def generate_law_data(loader, args, dataset='train'):
    device = args.device
    model_path = os.path.join(args.save_path, 'model.pth')
    model = torch.load(model_path)
    model.to(device)
    ...
    with torch.no_grad():
        for idx, (a, d, y) in enumerate(loader):
            u_mu, u_logvar = model.q_u(d.to(device), a.to(device), y.to(device))
            u_prev = model.reparameterize(u_mu, u_logvar)
            d_hard, d_cf_hard, y_hard, y_cf_hard = model.reconstruct_hard(u_prev, a.to(device))
            #i_hard = torch.cat((r_hard, d_hard), 1)
            #i_cf_hard = torch.cat((r_hard, d_cf_hard), 1)
            i_hard = d_hard
            i_cf_hard = d_cf_hard
            ...
            y_real = torch.cat((y_real, y), 0) if idx != 0 else y
            u = torch.cat((u, u_prev), 0) if idx != 0 else u_prev
            i_org = torch.cat((i_org, d.cuda()), 0) if idx !=0 else d.cuda()
        ...
        a_cf_all = a_all
        y_real = y_real.cpu().detach().numpy()
        i_org = i_org.cpu().detach().numpy()

    f_out_np = os.path.join(args.save_path, dataset)
    np.savez(f_out_np, input=i_f, input_cf=i_cf, y=y, y_cf=y_cf, a=a_all, a_cf=a_cf_all, u=u)
