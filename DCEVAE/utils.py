...
def make_loader(train_df, args):
    ...
    shuffle = np.random.permutation(a_valid.shape[0])
    a_valid2 = a_valid[shuffle]
    r_valid2 = r_valid[shuffle]
    d_valid2 = d_valid[shuffle]
    y_valid2 = y_valid[shuffle]
    ...
    shuffle = np.random.permutation(a_test.shape[0])
    a_test2 = a_test[shuffle]
    r_test2 = r_test[shuffle]
    d_test2 = d_test[shuffle]
    y_test2 = y_test[shuffle]
    ...
    shuffle = np.random.permutation(a_train.shape[0])
    a_train2 = a_train[shuffle]
    r_train2 = r_train[shuffle]
    d_train2 = d_train[shuffle]
    y_train2 = y_train[shuffle]

    train_set_r_tensor = torch.from_numpy(r_train)
    train_set_d_tensor = torch.from_numpy(d_train)
    train_set_a_tensor = torch.from_numpy(a_train)
    train_set_y_tensor = torch.from_numpy(y_train)
    train_set_r_tensor2 = torch.from_numpy(r_train2)
    train_set_d_tensor2 = torch.from_numpy(d_train2)
    train_set_a_tensor2 = torch.from_numpy(a_train2)
    train_set_y_tensor2 = torch.from_numpy(y_train2)
    train_set = utils.TensorDataset(train_set_r_tensor, train_set_d_tensor, train_set_a_tensor, train_set_y_tensor,\
                                    train_set_r_tensor2, train_set_d_tensor2, train_set_a_tensor2, train_set_y_tensor2)
    ...
    valid_set_a_tensor = torch.from_numpy(a_valid)
    valid_set_y_tensor = torch.from_numpy(y_valid)
    valid_set_r_tensor2 = torch.from_numpy(r_valid2)
    valid_set_d_tensor2 = torch.from_numpy(d_valid2)
    valid_set_a_tensor2 = torch.from_numpy(a_valid2)
    valid_set_y_tensor2 = torch.from_numpy(y_valid2)
    valid_set = utils.TensorDataset(valid_set_r_tensor, valid_set_d_tensor, valid_set_a_tensor, valid_set_y_tensor,\
                                    valid_set_r_tensor2, valid_set_d_tensor2, valid_set_a_tensor2, valid_set_y_tensor2)
    valid_loader = utils.DataLoader(valid_set, batch_size=args.batch_size, shuffle=False)
    ...
    test_set_a_tensor = torch.from_numpy(a_test)
    test_set_y_tensor = torch.from_numpy(y_test)
    test_set_r_tensor2 = torch.from_numpy(r_test2)
    test_set_d_tensor2 = torch.from_numpy(d_test2)
    test_set_a_tensor2 = torch.from_numpy(a_test2)
    test_set_y_tensor2 = torch.from_numpy(y_test2)
    test_set = utils.TensorDataset(test_set_r_tensor, test_set_d_tensor, test_set_a_tensor, test_set_y_tensor,
                                   test_set_r_tensor2, test_set_d_tensor2, test_set_a_tensor2, test_set_y_tensor2)
    test_loader = utils.DataLoader(test_set, batch_size=args.batch_size, shuffle=False)
